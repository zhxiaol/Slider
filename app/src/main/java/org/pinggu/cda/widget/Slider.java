...
import org.pinggu.cda.slider.R;
...
public class Slider extends View implements ThemeManager.OnThemeChangedListener{
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        //default color
        mPrimaryColor = colorControlActivated(context, 0xFF000000);
        mSecondaryColor = colorControlNormal(context, 0xFF000000);
    ...
    protected void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        if(mThumbBorderSize < 0)
            mThumbBorderSize = dpToPx(context, 2);

        if(mThumbRadius < 0)
            mThumbRadius = dpToPx(context, 10);

        if(mThumbFocusRadius < 0)
            mThumbFocusRadius = dpToPx(context, 14);
        ...
        if(textStyleDefined)
            mTypeface = load(context, familyName, style);
    ...
    private void measureText(){
        ...
        float maxWidth = (float)(mThumbRadius * Math.sqrt(2) * 2 - dpToPx(getContext(), 8));
        if(width > maxWidth){
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        int filledPrimaryColor = getMiddleColor(mSecondaryColor, isEnabled() ? mPrimaryColor : mSecondaryColor, mThumbFillPercent);
        ...
        if(mDiscreteMode){
            ...
            if(factor > 0){
                ...
                mPaint.setColor(getColor(mTextColor, factor));
            ...
            if(radius > 0) {
                ...
                canvas.drawCircle(x,y,radius+1,mPaint);
            }
        }
        else{
            float radius = isEnabled() ? mThumbCurrentRadius : mThumbCurrentRadius - mThumbBorderSize;
    ...
    public  int getColor(int baseColor, float alphaPercent){
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public  int colorControlActivated(Context context, int defaultValue){
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public  int colorControlNormal(Context context, int defaultValue){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
    ...
    public  Typeface load(Context context, String familyName, int style) {
        if(familyName != null && familyName.startsWith("asset:")){
                try {
                        final Typeface typeface = Typeface.createFromAsset(context.getAssets(), familyName.substring("asset:".length()));
                        return typeface;
                } catch (Exception e) {
